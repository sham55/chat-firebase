...
import * as FileSystem from "expo-file-system/legacy"; // ✅ using legacy import for Expo SDK 54
...
export default function Chat() {
  ...
  const markMessagesAsRead = async (msgs) => {
    try {
      const unread = msgs.filter(
        (m) => !m.readBy?.includes(auth.currentUser.email)
      );
  ...
  const handleSaveMedia = async (uri) => {
    try {
      ...
      const localFile = FileSystem.cacheDirectory + `chat_media_${Date.now()}.${ext}`;
      const { uri: downloadedUri } = await FileSystem.downloadAsync(uri, localFile);
  ...
  const confirmSaveMedia = (uri) => {
    Alert.alert("Save Media", "Do you want to save this media to your gallery?", [
      { text: "Cancel", style: "cancel" },
      { text: "Save", onPress: () => handleSaveMedia(uri) },
    ]);
  };

  /** 🔹 Render image and video messages */
